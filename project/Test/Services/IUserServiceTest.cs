        ...
        public void TestFollowUser()
        {
            using (var scope = new TransactionScope())
            {
                var userId1 = userService.RegisterUser(loginName, clearPassword,
                    new UserProfileDetails(firstName, lastName, email, language, country));
                UserProfile user1 = userProfileDao.Find(userId1);

                var userId2 = userService.RegisterUser("usuario2", clearPassword,
                    new UserProfileDetails(firstName, lastName, email, language, country));
                UserProfile user2 = userProfileDao.Find(userId2);

                long followId = userService.FollowUser(userId1, "usuario2");

                Assert.IsTrue(followDao.ExistsByUserId1AndUserId2(userId1, userId2) == followId);
                Assert.IsTrue(user1.numFollows == 1 && user2.numFollowers == 1);

                followId = userService.UnFollowUser(userId1, "usuario2");

                Assert.IsTrue(followDao.ExistsByUserId1AndUserId2(userId1, userId2) != followId);
                Assert.IsTrue(user1.numFollows == 0 && user2.numFollowers == 0);
            }
        }
        [TestMethod]
        [ExpectedException(typeof(InstanceNotFoundException))]
        public void TestFollowUserAlreadyFollow()
        {
            using (var scope = new TransactionScope())
            {
                var userId1 = userService.RegisterUser(loginName, clearPassword,
                ...
                userService.FollowUser(userId1, "usuario2");

                userService.FollowUser(userId1, "usuario2");
            }
        }

        //Falta crear test demostrando que mo va el unfollow

        [TestMethod]
        [ExpectedException(typeof(InstanceNotFoundException))]
        public void TestUnFollowUserAlreadyNotFollow()
        {
            using(var scope = new TransactionScope())
            {
                var userId1 = userService.RegisterUser(loginName, clearPassword,
                ...
                userService.UnFollowUser(userId1, "usuario2");
            }
        }
